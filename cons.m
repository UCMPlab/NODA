classdef cons
    ...
    methods (Static)
        ...
        function [pf, eden, rden]=penningfraction(n,den)
            Rn0=n.^2*cons.a0; % radius of Rydb. by bohr model using semi-classical method
            Rmax=1.8*(Rn0*2); % Robicheaux paper, within this distance, 90% penning ionize
            pf=1-exp(-4*pi*den*Rmax.^3/3); % proportion between 0 and Rmax
            eden=pf/2*den; % the den of electron produce is half the proportion (1e- per partner)
            rden=(1-pf)*den; % this is remaining density of rydbergs
        end
        ...
        function cc=fitgauss(t,data,tlim,tag)
            ...
            parfor i=1:a
                ...
                if strcmp(tag,'ramp')
                    [~,b]=findpeaks(y,'minpeakdistance',100,'minpeakheight',mean(y)/2,'sortstr','descend');
                    if length(b)>=1
                        b=b(1);
        ...
        function tau = rydperiod(n)
             ...
             tau = sqrt(16*pi^3*cons.epsilon*cons.me/(cons.el^2)*(cons.a0*1e-6*n^2)^3);
        end
